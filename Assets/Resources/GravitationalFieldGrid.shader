...
Shader "Custom/GravitationalFieldGrid"
{
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            {
                float4 position = UnityObjectToClipPos(input[0].position);

                AppendLine(input[0].color, position, UnityObjectToClipPos(input[0].up     ), line_stream);
                AppendLine(input[0].color, position, UnityObjectToClipPos(input[0].right  ), line_stream);
                AppendLine(input[0].color, position, UnityObjectToClipPos(input[0].forward), line_stream);
            }
